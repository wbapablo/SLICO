	...
	const string&				path) 
{
	...
	//fname = basename((char*)filename.c_str());
	//extn = strstr((char*)filename.c_str(),".")+1;
	fname = "abeja";
	string temp = fname;
...
void SLIC::PerformSLICO_ForGivenK(
	const unsigned int*			ubuff, //1
	const int					width, //2
	const int					height,//3
	int*						klabels,//4
	int&						numlabels, //5
	const int&					K,//6      required number of superpixels
	const double&				m, //7    weight given to spatial distance
	vector<double>& kssl,
	vector<double>& kssa,
	vector<double>& kssb,
	vector<double>& kssx,
	vector<double>& kssy)
{
	...
	int sz = m_width*m_height;
	//--------------------------------------------------
	// desde s a sz ** se va a recorrer toda la imagen de izq  a derecha y de arriba hacia abajo
	for (int s = 0; s < sz; s++) klabels[s] = -1;
	//--------------------------------------------------
	if (true) {
		DoRGBtoLABConversion(ubuff, m_lvec, m_avec, m_bvec);
	...
	vector<double> edgemag(0);
	//Detecta bordes con la derivada de la imagen
	if(perturbseeds) DetectLabEdges(m_lvec, m_avec, m_bvec, m_width, m_height, edgemag);
	//Genera orígenes de los superpixeles
	...
	for(int i=0; i<numlabels ; i++){
		kssl.push_back(kseedsl[i]);
		kssa.push_back(kseedsa[i]);
		kssb.push_back(kseedsb[i]);
		kssx.push_back(kseedsx[i]);
		kssy.push_back(kseedsy[i]);
	}
